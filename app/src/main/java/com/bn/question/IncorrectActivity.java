...
public class IncorrectActivity extends Activity implements OnClickListener{
    ...
    String[] questionInfo,piece,questionID;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        questionID = bundle.getStringArray(BundleConstant.SHORT_STR);//获取题目ID

        if(questionID!=null)
            maxPage = questionID.length;//学科所有的题目数量

        for(String s:questionInfo){
            String[] per = s.split("<#>");
            questionList.add(per);
        }

        //获得布局映射器
        inflater = LayoutInflater.from(IncorrectActivity.this);
        //创建Handler线程
        createHandlerThread();
        //设置该ContentView内容
        boolean flag = showQuestionContent(questionPage);
        //将所有与题目作答相关内容添加监听
        if(flag)
            setAnswerListener();
        //初始化工具栏
    ...
    {
        String result = NetConnectionUtil.queryQuestionCommentInQuestion(questionID[questionPage]);
     ...
     * @param i:题目编号
     */
    private boolean showQuestionContent(int i)
    {
        boolean hasFoundRecord = false;
        for(String[] ss:questionList)
        {
            if(ss.length>1&&ss[9].equals(questionID[i]))
            {
                piece = ss;
                hasFoundRecord = true;
                break;
        ...
        if(!hasFoundRecord)
        {
            setContentView(R.layout.question_blank);
            TextView subjectName = (TextView) findViewById(R.id.title_name);
            subjectName.setText(this.getIntent().getExtras().getString(BundleConstant.SUBJECT_NAME));
            return false;
        }
     ...
     * 初始化底下的工具栏按钮
     */
    private void initToolsBar(boolean hasFoundRecord)
    {
        //上一题
        findViewById(R.id.back_btn).setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(questionPage==0)
                    Toast.makeText(IncorrectActivity.this, "别按啦，这就是第一道题目喔。", Toast.LENGTH_SHORT).show();
                else
                {
                    questionPage--;
                    skipQuestion();
        ...
        textView.setText("移 除");

        findViewById(R.id.add_btn).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                deleteFromWrongSet();
                Toast.makeText(IncorrectActivity.this, "从错题集中移除成功", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        if(hasFoundRecord)
        {
            ...
            findViewById(R.id.comment_btn).setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    Bundle bundle = new Bundle();
                    bundle.putString(BundleConstant.INFO_STR, questionID[questionPage]);
                    startActivity(new Intent(IncorrectActivity.this, CommentQuestionActivity.class).putExtras(bundle));
                }
            });
        }
        ...
        findViewById(R.id.forward_btn).setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                if(questionPage==maxPage-1)
                    Toast.makeText(IncorrectActivity.this, "别按啦，这已经是最后一道题目啦。", Toast.LENGTH_SHORT).show();
     ...
     * 从错题集中删除
     */
    private void deleteFromWrongSet()
    {
        DataBaseUtil.createOrOpenDataBase(Constant.WRONG_SET);
        DataBaseUtil.insert("delete from wrong_set where question_id = "+"'"+questionID[questionPage]+"'");
